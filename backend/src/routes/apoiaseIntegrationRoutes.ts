 ...
 */
router.post('/:campaignSlug/integrations/telegram', authenticate, async (req, res) => {
  try {
    ...
    if (campaign.makerId.toString() !== userId) {
      return res.status(403).json({ error: 'Você não tem permissão para integrar esta campanha' });
    }
    ...
    const integrationServiceUrl = process.env.TELEGRAM_INTEGRATION_SERVICE_URL || 'http://localhost:3000';
    const callbackUrl = `${process.env.FRONTEND_URL || 'http://localhost:3000'}/profile/campaign?campaignId=${campaign._id}`;

    const redirectUrl = new URL(`${integrationServiceUrl}/integration/authorize`);
 ...
 */
router.get('/:campaignSlug/integrations/telegram', authenticate, async (req, res) => {
  try {
    const { campaignSlug } = req.params;
    const userId = req.user?.id;
 ...
 * @authentication Required
 */
router.delete('/:campaignSlug/integrations/telegram/:integrationId', authenticate, async (req, res) => {
  try {
    ...
    if (!integration || integration.campaignId.toString() !== campaign._id.toString()) {
